...
> **Status:** second track, running **in parallel** with the AskUbuntu track in
> [`../distilled/`](../distilled/) — not a replacement for it. Both are live.
> The two tracks share one independent variable (how negatives are selected) and one body of
> literature ([`distilled/negative-pair-research/`](../../distilled/negative-pair-research/));
...
| **RQ2** | Does the *method* used to select negatives (random / keyword-similar / semantically-similar) meaningfully change the result? | **Not established for code.** Established-ish for NLP text | [`FIVE_KEY_PAPERS.md`](../../distilled/negative-pair-research/FIVE_KEY_PAPERS.md). The cross-paper finding is `Random ≪ {BM25, Semantic}`, with the winner between the last two depending on label completeness |
...
* **H0b (inverted-U):** `N2 > N1` but `N3 < N2`. Hardness helps, then hurts. **This is the
  prediction the literature supports** — see [`FIVE_KEY_PAPERS.md`](../../distilled/negative-pair-research/FIVE_KEY_PAPERS.md):
...
| Shared | The independent variable; the three strategies; the hardness check; the false-negative measurement; the literature in `distilled/negative-pair-research/` | |
