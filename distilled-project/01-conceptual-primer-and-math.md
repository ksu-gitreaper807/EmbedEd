...
* **Hard negatives are contaminated.** A "hard negative" is a near-miss — a non-duplicate the model currently thinks *is* a duplicate. But the nearer a miss is, the likelier it is an *unlabelled duplicate* — a false negative. [RocketQA](https://aclanthology.org/2021.naacl-main.466/) solves this with **denoised hard negatives** (a cross-encoder filters suspicious candidates); [Robinson et al. (2021)](https://openreview.net/forum?id=S4nZh4WBHxq) state the two governing principles: a useful negative must (1) genuinely *be* a negative and (2) currently be *believed* similar.
